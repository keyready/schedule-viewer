...
export class ParserService {
  getRectangleFromExcel(fileName: string, rectangleVertices: string) {
    ...
    selectedData.forEach((column) => {
      ...
      column.forEach((cell) => {
        if (/^\d+$/.test(String(cell))) {
          date = new Date((Number(cell) - (25567 + 2)) * 86400 * 1000);
          ...
          result[realIndex].jobs.push(`Тип занятия: ${type}, дисциплина: ${discipline}, аудитория: ${room}`);
        }

        if (date.getDay() === 6) {
          if (result[realIndex].jobs.length >= 3) {
            result[realIndex].jobs.push('Тип занятия: хоз. день, дисциплина: хоз. день, аудитория: Убежище');
            result[realIndex + 1].date = new Date(new Date(result[realIndex].date).getTime() + 24 * 60 * 60 * 1000);
            for (let i = 0; i < 4; i += 1) {
              result[realIndex + 1].jobs.push('Тип занятия: Выходной день, Выходной день, аудитория: Каз.63');
            }
            realIndex += 2;
          }
  ...
  getRange(fileName: string, rectangleVertices: string) {
    const workbook = XLSX.readFile(fileName);
    const sheetNameList = workbook.SheetNames;
    const worksheet = workbook.Sheets[sheetNameList[0]];
    const vertices = rectangleVertices.split(':').map((vertex) => XLSX.utils.decode_cell(vertex));
    ...
    const str: any[][] = [];
    for (let i = 0; i < selectedData.length; i += 1) str.push([]);

    selectedData.filter((arr) => arr.length > 0 && arr.some((el) => el !== undefined)).forEach((column, index) => {
      column.forEach((cell) => {
        str[index].push(cell);
      });
    });
    ...
    const subjects: { abbr: any; title: any; lectern: number; trainer: any }[] = [];
